...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QFrame, QScrollArea, QApplication, \
    QMessageBox, QCheckBox, QLineEdit, QRadioButton, QTextEdit
...
class JanelaControleInstancias(QWidget):
    ...
    def abrir_todos(self):
        try:
            self.controller.reabrir_instancia("servidor", len(self.lista_programas), self.base)
            for programa in self.lista_programas:
                self.abrir_programa(
                    programa,
    ...
    def confirmar(self):
        ...
        if self.radio_netreport.isChecked() and self.controller.SACs_abertos:
            for programa in self.lista_programas:
                ...
                thread_monitoramento = threading.Thread(target=self.controller.monitorar_erros, args=(pid,))
                thread_monitoramento.daemon = True
                thread_monitoramento.start()

                try:
                    self.controller.automacao_netreport(pid, data, automatizado, lista_scripts)
                except Exception as e:
            ...
            for programa in self.lista_programas:
                ...
                try:
                    self.controller.automacao_processamento_geral(pid, data, automatizado, self.checkboxes_marcadas, lista_scripts)
                except Exception as e:
